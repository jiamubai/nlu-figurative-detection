...
def train_with_vad(model, optimizer, scheduler, loss_function, epochs,       
    ...
    for epoch in range(epochs):
        ...
        for step, batch in enumerate(train_dataloader): 
            ...
            correct += pred.eq(batch_labels.data.view_as(pred)).sum().data
            optimizer.zero_grad()
            loss.backward()
...
# load dataset
train_fl = pd.read_csv("sar_and_meta_train.csv")[:1000]
...
# load tokenizer 
if pretrain_vad:
    tokenizer = AutoTokenizer.from_pretrained('model_vad_5epoc_16batch_token')
else:
...
if pretrain_vad:
    model = torch.load('model_vad_5epoc_16batch') 
